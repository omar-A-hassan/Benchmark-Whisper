...
class Datasets(Enum):
    ...
    MASRI_SPEECH = "MASRI_SPEECH"
    MASRI_SPEECH_TRANSLATE = "MASRI_SPEECH_TRANSLATE"
...
class Dataset(object):
    ...
    def create(
        ...
        elif x is Datasets.MASRI_SPEECH:
            return MasriSpeechDataset(folder, language, punctuation, normalizer)
        elif x is Datasets.MASRI_SPEECH_TRANSLATE:
            return MasriSpeechTranslateDataset(folder, language, punctuation, normalizer)
        else:
...
class MasriSpeechDataset(Dataset):
    """
    MasriSpeech dataset for Arabic speech recognition
    Expects pre-processed dataset with audio files and CSV containing transcriptions and translations
    """
    SUPPORTED_LANGUAGES = [Languages.AR]
    SUPPORTS_PUNCTUATION = False

    def __init__(self, folder: str, language: Languages, punctuation: bool, normalizer: Normalizer):
        super().__init__(language, punctuation, Datasets.MASRI_SPEECH.value)

        self._language = language
        self._data = []

        # Expect a metadata.csv file with columns: audio_path, transcription, translation (optional)
        metadata_path = os.path.join(folder, "metadata.csv")

        if not os.path.exists(metadata_path):
            raise ValueError(
                f"MasriSpeech dataset requires a metadata.csv file in {folder}. "
                "The CSV should have columns: audio_path, transcription[, translation]"
            )
            ...
            for row in reader:
                ...
                try:
                    normalized_transcript = normalizer.normalize(
                        transcription,
                        raise_error_on_invalid_sentence=False
                    )
                    self._data.append((audio_path, normalized_transcript))
                except RuntimeError:
                    continue
    ...
    def __str__(self) -> str:
        return f"MasriSpeech {self._language.value}"


class MasriSpeechTranslateDataset(Dataset):
    """
    MasriSpeech dataset for Arabic to English translation evaluation
    Uses English translations as reference instead of Arabic transcriptions
    """
    SUPPORTED_LANGUAGES = [Languages.AR]  # Source language is Arabic
    SUPPORTS_PUNCTUATION = False

    def __init__(self, folder: str, language: Languages, punctuation: bool, normalizer: Normalizer):
        super().__init__(language, punctuation, Datasets.MASRI_SPEECH_TRANSLATE.value)

        self._language = language
        self._data = []

        # Use English normalizer for translation references
        from normalizer import EnglishNormalizer
        self._english_normalizer = EnglishNormalizer(keep_punctuation=False, punctuation_set="")

        # Expect a metadata.csv file with columns: audio_path, transcription, translation
        metadata_path = os.path.join(folder, "metadata.csv")

        if not os.path.exists(metadata_path):
            raise ValueError(
                f"MasriSpeech translation dataset requires a metadata.csv file in {folder}. "
                "The CSV should have columns: audio_path, transcription, translation"
            )
        ...
        with open(metadata_path, encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                # Use translation as reference (not transcription)
                ...
                try:
                    ...
                    normalized_translation = self._english_normalizer.normalize(
                        translation,
                        raise_error_on_invalid_sentence=False
                    )
                    self._data.append((audio_path, normalized_translation))
                except RuntimeError:
                    continue
    ...
    def __str__(self) -> str:
        return f"MasriSpeech Translate {self._language.value}→EN"
